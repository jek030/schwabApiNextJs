...
const formatterVol = new Intl.NumberFormat('en-US', {
  ...
  return (
    ...
    <div className="grid grid-rows-[20px_1fr_20px]  justify-items-left  min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      ...
      <p className={` text-l text-gray-800  md:leading-normal`}>
        <strong>Fundamentals</strong><br></br>
        {"Symbol: " + ticker} <br></br>
        { accounts[ticker].reference.description}<br></br>
      ...
      <p className={` text-l text-gray-800  md:leading-normal`}>
        ...
        {"After hours % change: " + formatter.format(accounts[ticker].quote.postMarketPercentChange)} <br></br>
          ...
          <Link className="mt-4 rounded-md bg-purple-500 px-4 py-2 text-sm text-white transition-colors hover:bg-blue-400"
           href={yahooURL}>{"Yahoo Finance"}</Link>
      </main>
